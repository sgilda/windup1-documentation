...
After you run the assembly:assembly command, move the jboss-windup-[VERSION]-bin directory to wherever you want to "install" Windup on your computer. 
...
**Description:** the following will run Windup against all Zip type archives in the folder following "-input", and will analyze classes in the org.tiaa and org.another packages. **Note:** the "-output" parameter is ignored in batch mode.
...
**Description:** the following will run Windup against the specific Zip following the "-input", and the report will be output in the "-output" directory.
    ...
    java -jar windup-cli.jar -input C:\Users\bdavis\Projects\migrations\tiaa\example.ear \
        -output C:\Users\bdavis\Projects\migrations\tiaa\example-doc -javaPkgs org.tiaa:org.another
